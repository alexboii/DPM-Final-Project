...
public class Navigation {
	...
	public double[] scanObject(){
		...
		//First edge
		while ((!wall) && (us.getDistance() < 
				Math.abs((initialDistance / Math.abs(Math.cos(Math.toRadians(odometer.getTheta())))) 
		...
		data = results; //static variable contains last set of results for print&debug
		return results;
	}
...
//					
	...
	public void goForward(double distance) {
		this.travelTo(
				odometer.getX()
						+ Math.cos(Math.toRadians(this.odometer.getTheta()))
						* distance,
				odometer.getY()
						+ Math.sin(Math.toRadians(this.odometer.getTheta()))
						* distance);
