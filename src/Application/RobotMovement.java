 ...
 * our search algorithm to find an object in the vecinity. If the object is a
 * wooden block, it avoids it, goes to the next waypoint and keeps looking for
 * objects.
 * 
 * If the found object is a blue block, it activates the claw mechanism to catch
 * it. If it is not the first blue block the robot encounters, previously picked
 * up blocks are placed on top of the new block. If the maximum amount of blue
 * blocks is not reached, it goes to the next way point and scans again.
 * Otherwise, robot goes to drop off zone, drops the blocks, and goes back to
 * its position.
 * 
 * 
...
public class RobotMovement extends Thread {
	...
	// CLASS CONSTANTS
	public static final int wayPointCounter = 0;
	public static final int STACK_HEIGHT = 2;
	public static final int ROTATE_SPEED = 76;
	private static final double TILE = 30.48;
	private static final int TURN_ANGLE_1 = 45;
	private static final int TURN_ANGLE_2 = 180;
	private static final int TURN_ANGLE_3 = 90;
	...
	private static final int SECOND_DISTANCE_SCAN = (int) (1.5 * TILE);
	private static final int DISTANCE_APPROACH_THRESHOLD = 5;
	private static final int ADDITION_SLEEP_TIME = 15;
	private static final int CLAW_MOTOR_ACCELERATION = 200;
	...
	public RobotMovement(Odometer odometer, Navigation navigator, USPoller usPollerLow, USPoller usPollerHigh,
		...
		clawMotor.setAcceleration(CLAW_MOTOR_ACCELERATION);
	...
	public void run() {
		...
		navigator.turnTo(TURN_ANGLE_3, true);
	...
	/**
	 * Calculates the middle of the the designated drop off zone & goes to that
	 * point.
	 */
	...
	/**
	 * Calculates X coordinates of a point based on distance from robot and
	 * angle
	 * 
	 * @param distance
	 *            to desired point
	 * @param angle:
	 *            heading where the point is located relative to the robot
	...
	/**
	 * Calculates Y coordinates of a point based on distance from robot and
	 * angle
	 * 
	 * @param distance
	 *            to desired point
	...
	/**
	 * Robot goes to next way point. The next way point is located along the
	 * line between the robot's current position and the drop off zone.
	 */
	...
	/**
	 * The robot rotates in its axis starting at theta=initialAngle and
	 * finishing at theta = finalAngle While scaning, every ADDITION_SLEEP_TIME
	 * miliseconds, a Vector object is created with the lower US sensor reading
	 * as length of the vector and the odometer's theta as the angle. When scan
	 * is done, the Vector with the smallest length is selected. Robot turns to
	 * that Vector's angle and moves forward by a distance equal to (Vector's
	 * length - SCAN_DISTANCE_THRESHOLD). Once in front of the object, the robot
	 * scans it. If it's a blue block, it picks it up and either keeps scanning
	 * or goes to the drop off zone. If object is a wooden block, the robot
	 * avoids it and goes to a new way point.
	 * 
	 * @param initialAngle
	 *            Starting angle for scan
	 * @param finalAngle
	 *            Ending angle for scan
	 */
	public void findObject(double initialAngle, double finalAngle) {
		...
		if ((list_of_vectors.get(0).getDistance()) < DISTANCE_SCAN_THRESHOLD) {
			...
			// If lower ultrasonic sensor detects the object, approach it based
			// on that reading
			...
			if (blockProperties[0] == 1) {
				...
				// If supposed blue block is close & still looking for blue
				// objects
				if (blockProperties[1] < DISTANCE_APPROACH_THRESHOLD) {
					if (blue_counter < STACK_HEIGHT) {
		...
		} else { // objects not close enough but in sight, the robot approaches
					// it & scans again
	...
	/**
	 * This method picks up a block. If it's not the first block, previously
	 * picked up blocks are placed on top of the new block.
	 */
