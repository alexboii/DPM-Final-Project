...
public class OdometerCorrection {
	...
	// return theta >= ONE_QUARTER_PI && theta < THREE_QUARTER_PI || theta >=
	// FIVE_QUARTER_PI && theta < SEVEN_QUARTER_PI;
	// }
	...
	public void CorrectorFormula() {
		if (!this.odometer.isRotating()
			...
			if (Math.abs(sensorYOffset) > Math.abs(sensorXOffset)) {
				// if (theta >= ONE_QUARTER_PI && theta < THREE_QUARTER_PI ||
				// theta >= FIVE_QUARTER_PI && theta < SEVEN_QUARTER_PI) {
